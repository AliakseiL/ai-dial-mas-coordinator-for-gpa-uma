    ...
    async def chat_completion(self, request: Request, response: Response) -> None:
        ...
        try:
            with response.create_single_choice() as choice:
                agent = MASCoordinator(
                    endpoint=DIAL_ENDPOINT,
                    deployment_name=DEPLOYMENT_NAME,
                    ums_agent_endpoint=UMS_AGENT_ENDPOINT
                )
                await agent.handle_request(request=request, choice=choice)
        except Exception as e:
            logger.error(f"Error in MASCoordinatorApplication: {str(e)}")
            raise e
...
app.add_chat_completion(
    deployment_name="mas-coordinator", impl=agent_app
)
